...
/**
 * CLASSIFY DATASET
 * Identifies the source, topic, and geography for the semantic layer.
 */
export function classifyDataset(data, schema) {
  const headers = schema.allHeaders || Object.keys(data[0] || {});
  const headerText = headers.join(' ').toLowerCase();
  const sampleText = data.slice(0, 10).map(r => Object.values(r).join(' ')).join(' ').toLowerCase();

  const isGWI = isGWISheet(headers, data);
  const isKWP = isKeywordPlan(data);
  const isTrends = headerText.includes('interest over time') || headerText.includes('search interest') || (headers.includes('Date') && data.length > 5 && headers.some(h => h.toLowerCase().includes('category')));
  
  // New Source Detection
  const isHelium10 = headerText.includes('asin') || headerText.includes('bsr') || (headerText.includes('sales') && headerText.includes('revenue') && headerText.includes('brand'));
  const isSimilarWeb = headerText.includes('bounce rate') || headerText.includes('total visits') || (headerText.includes('traffic source') && headerText.includes('domain'));
  const isKonnect = headerText.includes('sentiment') || headerText.includes('mention') || (headerText.includes('reach') && headerText.includes('author'));

  let source = 'Other';
  let topic = 'Other';
  let grain = 'Other';
  let geography = 'Global';
  let metricType = 'Value';

  if (isGWI) {
    source = 'GWI';
    topic = headerText.includes('social media') ? 'TimeSpent' : 'Culture';
    grain = 'SegmentByAudience';
    metricType = 'ShareOfAudience';
  } else if (isKWP) {
    source = 'GoogleAds';
    topic = 'KeywordPlan';
    grain = 'SegmentByKeyword';
    metricType = 'SearchVolume';
  } else if (isTrends) {
    source = 'GoogleTrends';
    topic = 'SearchDemand';
    grain = 'TimeSeries';
    metricType = 'InterestIndex';
  } else if (isHelium10) {
    source = 'Helium10';
    topic = 'SalesPerformance';
    grain = 'SegmentByProduct';
    metricType = 'Revenue';
  } else if (isSimilarWeb) {
    source = 'SimilarWeb';
    topic = 'TrafficYield';
    grain = 'SegmentByChannel';
    metricType = 'Visits';
  } else if (isKonnect) {
    source = 'Konnect';
    topic = 'SocialSentiment';
    grain = 'SegmentByPlatform';
    metricType = 'Engagement';
  }

  // Detect Geography
  const geoMarkers = [
    { name: 'India', markers: ['india', 'in', 'hindi'] },
    { name: 'UK', markers: ['uk', 'united kingdom', 'london'] },
    { name: 'US', markers: ['us', 'usa', 'united states', 'america'] },
    { name: 'UAE', markers: ['uae', 'emirates', 'dubai', 'abu dhabi'] }
  ];

  for (const g of geoMarkers) {
    if (headerText.includes(g.name.toLowerCase()) || g.markers.some(m => sampleText.includes(m))) {
      geography = g.name;
      break;
    }
  }
  ...
  // Detect Period
  let period = 'Current';
  const periodMatch = sampleText.match(/q[1-4]\s*202\d/) || sampleText.match(/202\d/);
  if (periodMatch) period = periodMatch[0].toUpperCase();

  // Entity Extraction (Deep understanding for probabilistic joining)
  const entities = {
    asins: Array.from(sampleText.matchAll(/b0[a-z0-9]{8}/g)).map(m => m[0].toUpperCase()),
    brands: [],
    domains: Array.from(sampleText.matchAll(/[a-z0-9.-]+\.(com|org|net|io|in|uk|ae)/g)).map(m => m[0])
  };
  ...
  return {
    ...
    grain,
    entities,
    primaryKeys: schema.categorical.slice(0, 2)
  };
}
