export function isGWISheet(headers: string[], rows: Record<string, unknown>[]): boolean {
  ...
  const topRowsText = rows.slice(0, 10)
    .map(r => Object.values(r).join(' '))
    .join(' ')
    .toLowerCase();
  const looksLikeGWI =
    topRowsText.includes('source: gwi') ||
    topRowsText.includes('time spent on social media');

  return (
    hasAudience && hasDataPoint && hasUniverse &&
    hasIndex && hasResponses && hasShortLabel &&
    hasAttributes && looksLikeGWI
  );
}
...
export function parseGWIQuestion(rows: Record<string, unknown>[]): GWIMeta {
...
export function tidyGWIData(
  headers: string[],
  rows: Record<string, unknown>[]
): { meta: GWIMeta; data: TidyRow[] } {
  ...
  rows.forEach(row => {
    ...
    metricCols.forEach(mCol => {
      const val = parseFloat(String(row[mCol]));
      if (isNaN(val)) return;
      tidy.push({ attribute, label, metric: mCol, value: val, question: meta.question });
    });
  });
