...
export function tidyGwiTimeSpent(
  ...
  const attrCol = headers.findIndex(h => String(h || '').toLowerCase().includes('attributes')) || 1;
  const labelCol = headers.findIndex(h => String(h || '').toLowerCase().includes('short label')) || 2;
