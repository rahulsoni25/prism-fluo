...
export function isGWISheet(headers, rows) {
  ...
  const hasAudience = headerText.includes('audience %');
  ...
  // GWI columns often have "Audience %" or "Data Point %" multiple times
  const metricCount = (headerText.match(/audience|data point|universe|index/g) || []).length;

  return (
    (hasAudience && hasUniverse && metricCount >= 3) ||
    (hasAudience && hasIndex && metricCount >= 3) ||
    (headerText.includes('source: gwi'))
  );
}
