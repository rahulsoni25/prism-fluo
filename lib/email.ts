 ...
 * Logs to console when key is absent (dev / unconfigured).
 */
...
export async function sendEmail(payload: EmailPayload): Promise<boolean> {
  ...
  if (!apiKey) {
    console.log('[Email] RESEND_API_KEY not configured — skipping:', payload.subject, '→', payload.to);
    return false;
  }
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from:     payload.from    ?? 'PRISM Intelligence <onboarding@resend.dev>',
...
function detailRow(label: string, value: string): string {
  return `<div style="display:flex;justify-content:space-between;align-items:flex-start;padding:10px 0;border-bottom:1px solid #F1F5F9">
    <span style="font-size:13px;color:#64748B;font-weight:500;flex-shrink:0">${label}</span>
    <span style="font-size:13px;color:#0F172A;font-weight:700;text-align:right;margin-left:16px">${value}</span>
  </div>`;
}

/**
 * Build and send a "New Brief Submitted" notification to rahul@fluodigital.com.
 */
...
<html lang="en">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>New PRISM Brief</title></head>
...
<div style="max-width:600px;margin:0 auto">
  ...
  <div style="background:linear-gradient(135deg,#0F172A 0%,#1E1B4B 60%,#1E3A8A 100%);border-radius:16px 16px 0 0;padding:32px;text-align:center">
    <div style="width:52px;height:52px;border-radius:14px;background:linear-gradient(135deg,#2563EB,#7C3AED);display:inline-flex;align-items:center;justify-content:center;font-size:22px;font-weight:900;color:#fff;margin-bottom:14px;letter-spacing:-1px">P</div>
    <div style="color:#fff;font-size:20px;font-weight:800;letter-spacing:-.4px;margin-bottom:4px">New Brief Submitted</div>
  ...
  <div style="background:#fff;padding:28px 32px;border-left:1px solid #E2E8F0;border-right:1px solid #E2E8F0">
    ...
    <div style="background:#EFF6FF;border:1px solid #BFDBFE;border-radius:12px;padding:14px 18px;margin-bottom:24px;display:flex;align-items:center;gap:14px">
      ...
      <div>
        ...
        <div style="font-size:12px;color:#2563EB;margin-top:1px">${submittedBy.email}</div>
      </div>
    </div>
    ...
    <!-- Status -->
    <div style="margin-top:22px;padding:14px 18px;background:linear-gradient(135deg,#FFF7ED,#FFFBEB);border:1px solid #FDE68A;border-radius:10px;display:flex;align-items:center;gap:10px">
      <span style="font-size:18px">⏳</span>
      <div>
        <div style="font-size:13px;font-weight:700;color:#92400E">Brief In Progress — Awaiting Data Upload</div>
        <div style="font-size:11px;color:#B45309;margin-top:2px">SLA will be calculated once data files are uploaded</div>
      </div>
    </div>
  </div>
  ...
  <div style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:0 0 16px 16px;padding:18px 32px;text-align:center">
    <div style="font-size:12px;color:#94A3B8">PRISM Intelligence Platform · <a href="https://prism-fluo.vercel.app" style="color:#2563EB;text-decoration:none">prism-fluo.vercel.app</a></div>
  </div>
...
/**
 * Notify rahul@fluodigital.com when a brief becomes Active (data uploaded + analysis ready).
 */
export async function sendBriefActiveEmail(
  ...
  const html = `<!DOCTYPE html>
<html lang="en">
<head><meta charset="utf-8"><title>Brief Active — PRISM</title></head>
<body style="font-family:'Inter',Arial,sans-serif;background:#F1F5F9;margin:0;padding:24px 12px">
<div style="max-width:600px;margin:0 auto">
  ...
  <div style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:0 0 16px 16px;padding:18px 32px;text-align:center">
    <a href="https://prism-fluo.vercel.app/dashboard" style="background:linear-gradient(135deg,#059669,#10B981);color:#fff;padding:10px 24px;border-radius:10px;font-size:13px;font-weight:700;text-decoration:none">View Dashboard →</a>
  </div>
</div>
</body>
</html>`;
