...
export const PLATFORMS_DATA = [
  {name:'Global Web Index (GWI)',icon:'👥',desc:'Audience profiling, media habits & psychographics',status:'fetching',pct:71,note:'Profiling 14.2M respondents…'},
  {name:'Comscore',icon:'📡',desc:'Digital reach, traffic & cross-platform measurement',status:'fetching',pct:56,note:'Mapping digital audience reach…'},
  {name:'SimilarWeb',icon:'🔍',desc:'Competitor web traffic & digital market intelligence',status:'fetching',pct:44,note:'Competitor analysis in progress…'},
  {name:'Google Trends',icon:'📈',desc:'Search trend analysis & seasonality patterns',status:'fetching',pct:82,note:'Fetching 24-month trend data…'},
  {name:'Google Insights Finder',icon:'🎯',desc:'Consumer intent signals & audience interests',status:'fetching',pct:28,note:'Establishing API connection…'},
  {name:'Brandwatch Sentiment',icon:'💬',desc:'Brand sentiment, social listening & share of voice',status:'fetching',pct:65,note:'Processing 2.3M brand mentions…'},
  {name:'Helium10',icon:'🛒',desc:'E-commerce, Amazon rankings & keyword intelligence',status:'fetching',pct:19,note:'Mining keyword intelligence…'},
  {name:'Google Keyword',icon:'🔑',desc:'Search volume, CPC & competitive keyword analysis',status:'fetching',pct:53,note:'Mapping high-intent keywords…'},
];
...
export const ID = {
  content: [
    ...
    },
    {
      ...
      chartExtra: {plugins:{legend:{display:true,position:'top',labels:{font:{size:10,family:'Inter'},padding:8,boxWidth:9,usePointStyle:true}}}}
    },
    {
    ...
    },
    {
      ...
      chartData: {
        ...
        datasets: [{label:'Voice Search Index',data:[100,118,142,189,234],borderColor:C.blue,backgroundColor:C.blueL,fill:true,tension:0.4,pointBackgroundColor:C.blue,pointRadius:4,pointHoverRadius:6}]
      }
    },
  ],
  commerce: [
    ...
    {
      title: 'Purchase journey loses 91% of shoppers between discovery and checkout',
      source: 'GWI + SimilarWeb', confidence: 88,
      obs: 'Of every 100 consumers who discover Nike India on social media, only 52 visit the brand website, 29 shortlist a product, 15 add it to cart, and just 9 complete a purchase. The sharpest drop — 48 in every 100 — happens between social discovery and the first website visit, exposing a critical social-to-DTC gap.',
      rec: 'Add Instagram and YouTube shoppable overlays to cut the discovery-to-site drop-off in half. Deploy a 3-touch retargeting sequence for product page viewers who didn\'t add to cart. A/B test a "Swipe up to shop" Reels format against standard link-in-bio to measure conversion lift.',
      stat: 'Only 9 in 100 social discoverers reach checkout — 91% funnel drop',
      lbl: 'Purchase Conversion Funnel — Social Discovery to Checkout',
      chartType: 'funnel',
      chartData: { labels: ['Social Discovery','Website Visit','Product Shortlist','Add to Cart','Purchase'], values: [100, 52, 29, 15, 9] }
    },
    {
  ...
  communication: [
    ...
    },
    {
      ...
      chartData: {
        ...
        datasets: [{label:'Search Volume',data:[145,162,98,76,72,68,75,82,91,178,210,168],borderColor:C.purple,backgroundColor:C.purpleL,fill:true,tension:0.4,pointBackgroundColor:C.purple,pointRadius:3,pointHoverRadius:6}]
      }
    },
    {
      ...
      lbl: 'Conversion Multiplier (bars) vs Authenticity Score trend (line) by Influencer Tier',
      chartType: 'combo',
      chartData: {
        labels: ['Celebrity >1M','Macro 200K–1M','Micro 50K–200K','Nano 10K–50K'],
        datasets: [
          {label:'Conversion ×',data:[1.0,1.8,2.7,3.1],backgroundColor:C.purple,borderRadius:5,borderSkipped:false},
          {label:'Authenticity ×',data:[1.0,1.6,2.2,2.7],borderColor:C.teal,borderWidth:2.5,tension:0.4},
        ]
      },
  ...
  culture: [
    {
      ...
      chartData: {
        labels: ['2021','2022','2023','2024','2025'],
        datasets: [{label:'% 3+ times/week',data:[52,55,60,64,67],borderColor:C.orange,backgroundColor:C.orangeL,fill:true,tension:0.4,pointBackgroundColor:C.orange,pointRadius:4,pointHoverRadius:6}]
      },
    ...
    {
      title: 'Urban Indian fitness audience scores high on performance — low on sustainability',
      source: 'GWI', confidence: 84,
      obs: 'Profiling Nike\'s core 18–34 Indian audience across five brand-fit dimensions reveals strong alignment on Performance (87/100) and Style Consciousness (82), but below-average scores on Sustainability Awareness (58) and Community Belonging (64). Aspiration for self-improvement (79) rounds out the profile.',
      rec: 'Lead with performance and style in all hero communication. Layer in community and sustainability as secondary messages — backed by specific product stories (recycled materials, training collectives) rather than generic brand-level CSR claims.',
      stat: 'Performance scores 29 points higher than Sustainability in the brand-fit index',
      lbl: 'Brand Fit Dimensions — Nike India Core Audience (Score out of 100)',
      chartType: 'radar',
      chartData: {
        labels: ['Performance','Style','Aspiration','Community','Sustainability'],
        datasets: [{
