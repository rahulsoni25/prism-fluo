...
export const PENDING_TASKS: PendingTask[] = [
  // ── HIGH ────────────────────────────────────────────────────────────
  {
    id:            'oauth-login-buttons',
    title:         'Re-enable OAuth login (Google + LinkedIn)',
    emoji:         '🔐',
    category:      'hidden-feature',
    criticality:   'high',
    dateDiscussed: '2026-05-24',
    context:       'OAuth buttons were hidden during the auth refactor because Google Cloud Console did not have redirect URIs registered for all environments (dev/preview/prod). New users currently can only sign up via email — friction on first-time onboarding.',
    status:        'parked',
    effort:        '~1 hr code + 30 min Google Console config',
    whereInCode:   'app/login/page.js:63',
    blockers:      [
      'Register https://prism-fluo.vercel.app/api/auth/oauth/google/callback in Google Cloud Console',
      'Register same for LinkedIn dev portal',
      'Add GOOGLE_CLIENT_ID + GOOGLE_CLIENT_SECRET env vars to Vercel (Production + Preview)',
      'Test the round-trip in incognito',
    ],
