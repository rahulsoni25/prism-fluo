...
/* Cache version — bump when the strategicRead generation logic changes in
   a way that older cached paragraphs should be invalidated. Cached records
   that don't match this version get regenerated on next load. */
const STRATEGIC_READ_VERSION = 2;
...
export async function GET(
  ...
  // ?regenerate=1 forces a fresh synthesis (bypasses cache, useful when
  // the user wants to see updated logic without re-running the analyzer).
  ...
  try {
    ...
    if (nuggets && (nuggets.keyword || nuggets.helium10 || nuggets.competition || nuggets.cultural || nuggets.trust)) {
      ...
      // Use cached strategicRead if it (a) exists, (b) matches the current
      // version, and (c) hasn't been force-regenerated by ?regenerate=1.
      const cachedVersion = Number(results.executiveSummary?._version ?? 0);
      const useCached = !forceRegenerate && cachedVersion === STRATEGIC_READ_VERSION;
      let strategicRead: string = useCached ? (results.executiveSummary?.strategicRead || '') : '';
      if (!strategicRead) {
      ...
      const summary = {
        ...
        strategicRead,
        actions,
        _version:      STRATEGIC_READ_VERSION,
      };
    ...
    // Legacy fallback path: honor version + regenerate flag
    if (results.executiveSummary?.strategicRead) {
      const cachedVersionL = Number(results.executiveSummary?._version ?? 0);
      if (!forceRegenerate && cachedVersionL === STRATEGIC_READ_VERSION) {
      ...
      // Stale cache (different version) → fall through and regenerate.
    }
